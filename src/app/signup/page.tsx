...
function SignupForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (data.session) {
      ...
      setInfo("ตรวจสอบอีเมลเพื่อยืนยันการสมัคร แล้วเข้าสู่ระบบอีกครั้ง");
    }
  }
  ...
  return (
    <div className="w-full max-w-sm space-y-6">
      <div className="text-center">
        ...
        <p className="text-sm text-[hsl(var(--muted-foreground))]">สมัครสมาชิก</p>
      </div>
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium mb-1">
            รหัสผ่าน <span className="text-[hsl(var(--muted-foreground))]">(อย่างน้อย 6 ตัว)</span>
          </label>
          <input
            type="password"
            required
        ...
        {info && (
          <p className="text-sm text-[hsl(var(--primary))]">{info}</p>
        )}
        <button
          type="submit"
          disabled={loading}
      ...
      <p className="text-center text-sm text-[hsl(var(--muted-foreground))]">
        มีบัญชีแล้ว?{" "}
        <Link
          href={`/login${next !== "/groups" ? `?next=${encodeURIComponent(next)}` : ""}`}
